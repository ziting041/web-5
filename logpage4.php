...
<?php
 // 啟用 session
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $sql = "SELECT * FROM user WHERE account='$account' AND pw='$pw'";
        ...
        if ($row = mysqli_fetch_assoc($result)) {
            ...
            // 跳轉到 manager.php
            header("Location: choose.html?account=" . urlencode($account));
            exit;
        } else {
            // 如果帳號或密碼錯誤，顯示錯誤訊息
            $error_message1 = "帳號或密碼錯誤";
        }
    }
?>
    ...
    <div class="wrapper">
        <form id="loginForm" action="logpage4.php" method="post">
            ...
            <div class="register-link">
                <p>還沒有帳號？ <a href="signup.html">註冊</a></p>
            </div>
            <?php
            // 如果有錯誤訊息，顯示在表單下方
            if ($error_message1) {
                echo "<div class='error-message1'>$error_message1</div>";
            }
            ?>
        </form>
    </div>
